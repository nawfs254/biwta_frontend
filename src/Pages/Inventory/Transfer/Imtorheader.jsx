...
const Imtorheader = () => {
  ...
  const handleSortChange = (field) => {
    setSortOrder((prevOrder) =>
      field === sortField && prevOrder === "asc" ? "desc" : "asc"
    );
    setSortField(field);
  };
    ...
    try {
      ...
      addFunction(data, endpoint, "POST", (response) => {
        if (response && response.xtornum) {
          setFormData((prev) => ({
            ...prev,
            xtornum: response.xtornum,
            xstatustor: response.xstatustor,
          }));
  ...
  const handleItemSelect = useCallback((item) => {
    ...
    setFormData((prev) => ({
      ...prev,
      xtornum: item.xtornum,
      xfwh: item.xfwh,
      xlong: item.xlong,
      xfwhdesc: item.xfwhdesc,
      xstatustor: item.xstatustor,
      xtwh: item.xtwh,
    }));
  }, []);
    ...
    if (window.confirm("Confirm This Requisition?")) {
      ...
      try {
        const response = await axiosInstance.post(
          "/api/imtorheader/confirmRequest",
          params
        );
              ...
              >
                ...
                >
                  ...
                  >
                    <Typography
                      variant="subtitle1"
                      sx={{ fontWeight: 400, fontSize: "1rem", pl: "5px" }}
                    >
                      Status:
                    </Typography>
                    <Typography
